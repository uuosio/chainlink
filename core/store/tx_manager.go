...
func (txm *EthTxManager) bumpGas(tx *models.Tx, attemptIndex int, blockHeight uint64) error {
	...
	for {
		if bumpedGasPrice.Cmp(txm.config.EthMaxGasPriceWei()) > 0 {
			...
			return fmt.Errorf("bumped gas price of %v would exceed maximum configured limit of %v, set by ETH_MAX_GAS_PRICE_WEI", bumpedGasPrice, txm.config.EthMaxGasPriceWei())
		}
