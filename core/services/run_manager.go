...
func promInstrumentJobRunUpdate(jobSpecID *models.ID, from models.RunStatus) func(*models.RunStatus) {
	return func(status *models.RunStatus) {
		if *status == from {
			return
		}
...
func (rm *runManager) CreateErrored(
	...
	defer promInstrumentJobRunUpdate(run.JobSpecID, run.Status)(&run.Status)
...
func (rm *runManager) Create(
	...
	defer promInstrumentJobRunUpdate(run.JobSpecID, models.RunStatusUnstarted)(&run.Status)
